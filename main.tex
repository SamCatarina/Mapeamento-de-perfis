...
No contexto deste trabalho, após o treinamento do modelo Random Forest, os valores SHAP são calculados utilizando o \textit{TreeExplainer}, um explicador otimizado para modelos baseados em árvores. O \textit{TreeExplainer} foi desenvolvido por Lundberg e colaboradores como um algoritmo especializado capaz de explorar diretamente a estrutura hierárquica e os caminhos decisórios das árvores do modelo. Em vez de estimar contribuições por meio de aproximações combinatórias — que possuem custo computacional exponencial — o \textit{TreeExplainer} utiliza algoritmos polinomiais que percorrem as regras de divisão das árvores, permitindo calcular contribuições individuais de forma rápida, precisa e matematicamente consistente. Assim, ele preserva as propriedades fundamentais dos valores de Shapley (como eficiência, simetria e aditividade), ao mesmo tempo em que torna viável a explicação de florestas com centenas de árvores. Enquanto a importância tradicional do Random Forest identifica quais atributos mais influenciam o modelo de maneira global, os valores SHAP permitem observar não apenas a magnitude dessa influência, mas também seu sentido (positivo ou negativo), bem como possíveis interações entre variáveis. Dessa forma, a combinação entre ambos fornece uma análise mais completa: primeiro identifica-se \textit{quais} variáveis são importantes; em seguida, com SHAP, investiga-se \textit{como} elas atuam na previsão.
...
\textbf{Valor Original} & \textbf{Codificado} & \textbf{Normalizado} \\
\hline
\raggedright maioria-valorizava & 0 & 0.00 \\
\raggedright alguns-valorizavam & 1 & 0.25 \\
\raggedright tratavam-neutro & 2 & 0.50 \\
\raggedright poucos-valorizavam & 3 & 0.75 \\
\raggedright desempenho-ignorado & 4 & 1.00 \\
\hline
\end{tabular}
...
\textbf{Valor Original} & \textbf{Codificado} & \textbf{Normalizado} \\
\hline
\raggedright muito-interesse & 0 & 0.00 \\
\raggedright algum-interesse & 1 & 0.33 \\
\raggedright pouco-interesse & 2 & 0.67 \\
\raggedright nenhum-interesse & 3 & 1.00 \\
\hline
\end{tabular}
...
\textbf{Valor Original} & \textbf{Codificado} & \textbf{Normalizado} \\
\hline
\raggedright nao-raremente & 0 & 0.00 \\
\raggedright raramente-responsabilidades & 1 & 0.33 \\
\raggedright as-vezes & 2 & 0.67 \\
...
Após a identificação das oito variáveis mais relevantes pelo modelo Random Forest com SHAP na análise geral, procedeu-se à formação de grupos (clusters) utilizando a técnica de \textit{clustering} hierárquico. O objetivo desta etapa foi verificar se os respondentes apresentavam padrões semelhantes de comportamento ou percepção, e se esses padrões permitiam a formação de perfis distintos de estudantes. As oito \textit{features} utilizadas como base para essa etapa foram: \textit{feelingExcludedTech}, \textit{activityGenderRestriction}, \textit{professionsByGenderOpinion}, \textit{teacherPerformanceValue}, \textit{preCollegeExactInterestLevel}, \textit{familyStudyIncentive}, \textit{stemCareerInterest} e \textit{childhoodHobbiesTime}. Essas variáveis foram selecionadas por representarem os fatores mais importantes para diferenciação entre os gêneros na análise anterior, o que justificou sua utilização como descritores dos possíveis perfis.
...
\begin{figure}[h!]
    ...
    \caption{Legenda}
    \includegraphics[width=0.9\textwidth]{geral_stem/dendrograma_geral_gender_stem.png}
    \label{fig:shap_masculino_future}
\end{figure}
...
\begin{figure}[h!]
    \centering
    \caption{Legenda}
    \includegraphics[width=0.9\textwidth]{geral_stem/cotovelo_geral_gender_stem.png}
    \label{fig:shap_masculino_future}
\end{figure}
\subsection{Estrutura e Tamanho dos Clusters}

Os dois grupos formados apresentam tamanhos distintos: o \textbf{Cluster 1} com 23 participantes e o \textbf{Cluster 2} com 65 participantes. A distribuição de gênero indica uma diferença marcante entre os grupos: o Cluster 1 possui maioria masculina (22 homens e apenas 1 mulher), enquanto o Cluster 2 apresenta uma composição mais equilibrada (27 mulheres e 38 homens). Esse desequilíbrio sugere, desde o início, que os clusters podem refletir diferenças de percepção associadas ao gênero e potencialmente relacionadas às barreiras ou estímulos vinculados ao interesse por STEM.
...
\subsubsection*{Cluster 1: Perfil com menor identificação com STEM e maior presença de sentimentos negativos}

O primeiro cluster reúne majoritariamente estudantes do gênero masculino. As características predominantes incluem:

\begin{itemize}
    \item maiores concentrações de respostas intermediárias ou negativas em \textit{preCollegeExactInterestLevel}, indicando baixo ou moderado interesse prévio por áreas de exatas;
    \item menor incentivo familiar para os estudos, concentrando valores mais baixos em \textit{familyStudyIncentive};
    \item níveis reduzidos de interesse em seguir carreira em STEM (\textit{stemCareerInterest});
    \item tempo de dedicação mais limitado a atividades de interesse pessoal na infância (\textit{childhoodHobbiesTime}), sugerindo menor contato prévio com atividades que tipicamente estimulam curiosidade científica e tecnológica;
    \item maior reprodução de estereótipos de gênero associados a profissões, evidenciado pela concentração elevada de valores altos em \textit{professionsByGenderOpinion}.
\end{itemize}

Assim, o Cluster 1 corresponde a um perfil menos próximo das áreas STEM, marcado por visões mais rígidas sobre profissões por gênero, menor incentivo familiar, menor interesse prévio e menor engajamento espontâneo em atividades de caráter exploratório durante a infância. Essa combinação está alinhada com a predominância masculina no grupo, dado que os meninos, conforme observado na análise de SHAP, tendem a reproduzir mais estereótipos de gênero e demonstram menor incentivo familiar a continuar os estudos.

\subsubsection*{Cluster 2: Perfil mais orientado ao interesse e permanência em STEM}

O Cluster 2, mais numeroso e composto por uma distribuição de gênero mais balanceada, apresenta padrões diferentes:

\begin{itemize}
    \item maiores níveis de interesse prévio em exatas (\textit{preCollegeExactInterestLevel});
    \item elevados índices de incentivo familiar aos estudos (\textit{familyStudyIncentive}), reforçando achados prévios de que meninas tendem a receber mais incentivo para estudar e permanecer na escola, enquanto meninos recebem mais estímulo para trabalhar;
    \item maior tempo dedicado a hobbies e atividades na infância (\textit{childhoodHobbiesTime}), o que pode estar associado a maior autonomia e maior espaço para desenvolvimento de interesses pessoais;
    \item maior interesse declarado em seguir carreira em STEM (\textit{stemCareerInterest});
    \item menor concordância com estereótipos de gênero em profissões, concentrando valores mais baixos em \textit{professionsByGenderOpinion}.
\end{itemize}

Esse conjunto sugere um perfil mais favorável à aproximação com STEM, tanto em termos de interesse quanto de percepções e contextos de incentivo. Esse grupo mostra características compatíveis com estudantes que se sentem mais confortáveis e motivados em relação à área, além de contarem com mais suporte familiar e apresentarem menor internalização de estereótipos de gênero.

\subsection{Síntese Interpretativa dos Grupos}

Os resultados indicam que a formação dos clusters reflete diferenças estruturais relevantes entre os estudantes, especialmente no que diz respeito ao interesse por STEM, às percepções de estereótipos de gênero e ao incentivo recebido ao longo da trajetória educativa.

De forma geral, o \textbf{Cluster 1} representa um grupo mais distante das áreas de STEM, com menor interesse e menores incentivos, enquanto o \textbf{Cluster 2} representa um perfil mais envolvido e motivado. A presença de estereótipos de gênero mais fortes no Cluster 1 e de maior incentivo familiar no Cluster 2 reforça achados já observados nas análises anteriores com SHAP.

Além disso, a distribuição de gênero entre os clusters sugere que fatores socioculturais continuam desempenhando papel crucial na forma como homens e mulheres percebem e se relacionam com as áreas STEM, evidenciando a necessidade de ações de incentivo e acolhimento especialmente voltadas para estudantes que se encontram mais próximos do perfil identificado no Cluster 1.

\subsection{Visualização Comparativa dos Perfis por Cluster}

Além do dendrograma e das análises numéricas apresentadas, a Figura~\ref{fig:radar_clusters} mostra o gráfico do tipo radar (ou gráfico aranha) com as médias normalizadas das oito \textit{features} que compuseram o processo de clusterização. Essa visualização permite comparar de forma intuitiva o perfil médio dos dois grupos identificados, destacando dimensões nas quais há maior divergência entre os clusters.

Observa-se que os dois grupos apresentam padrões distintos em praticamente todas as variáveis analisadas, o que reforça a consistência da divisão em dois clusters identificada pelo método hierárquico e confirmada pelos critérios do cotovelo e da silhueta.

No caso do \textbf{Cluster 1}, verifica-se que esse grupo apresenta valores mais elevados em \textit{professionsByGenderOpinion} e \textit{feelingExcludedTech}. Isso indica maior concordância com estereótipos de gênero associados a profissões e maior sensação de exclusão em ambientes de tecnologia, respectivamente. Esses resultados são coerentes com o perfil anteriormente descrito para esse cluster, predominantemente masculino e com menor identificação com a área de STEM.

Por outro lado, o \textbf{Cluster 2} apresenta valores mais altos em variáveis que representam incentivo e interesse em STEM, como \textit{familyStudyIncentive}, \textit{preCollegeExactInterestLevel} e \textit{stemCareerInterest}. O comportamento dessas variáveis sugere um grupo com maior motivação educacional, maior incentivo familiar para estudar e maior interesse consolidado em seguir carreira na área. Além disso, o Cluster 2 apresenta níveis mais elevados de \textit{childhoodHobbiesTime}, indicando maior tempo dedicado a atividades formativas ou exploratórias na infância, o que pode estar relacionado ao desenvolvimento de habilidades ou interesses associados à área científica e tecnológica.

Essa visualização contribui para confirmar e sintetizar os padrões previamente observados: enquanto o Cluster 1 se aproxima de um perfil de menor engajamento com STEM e maior manifestação de estereótipos de gênero, o Cluster 2 mostra características mais favoráveis à permanência e ao interesse em carreiras científicas e tecnológicas. Dessa forma, o gráfico aranha funciona como uma ferramenta comparativa poderosa para compreender a estrutura e as diferenças qualitativas entre os grupos identificados no processo de clusterização.
%====================================================================
...
%====================================================================
% Tabela 4 — teacherPerformanceValue
%====================================================================
\begin{table}[H]
\centering
\caption{Distribuição da feature \textit{teacherPerformanceValue} por cluster}
\label{tab:teacherPerformanceValue}
\begin{tabular}{lccccc}
\toprule
\textbf{Cluster} & \textbf{0.00} & \textbf{0.25} & \textbf{0.50} & \textbf{0.75} & \textbf{1.00} \\
\midrule
...
%====================================================================
% Tabela 5 — preCollegeExactInterestLevel
%====================================================================
\begin{table}[H]
\centering
\caption{Distribuição da feature \textit{preCollegeExactInterestLevel} por cluster}
\label{tab:preCollegeExactInterestLevel}
\begin{tabular}{lcccc}
\toprule
\textbf{Cluster} & \textbf{0.00} & \textbf{0.33} & \textbf{0.66} & \textbf{1.00} \\
\midrule
...
%====================================================================
% Tabela 6 — familyStudyIncentive
%====================================================================
\begin{table}[H]
\centering
\caption{Distribuição da feature \textit{familyStudyIncentive} por cluster}
\label{tab:familyStudyIncentive}
\begin{tabular}{lccccc}
\toprule
\textbf{Cluster} & \textbf{0.00} & \textbf{0.25} & \textbf{0.50} & \textbf{0.75} & \textbf{1.00} \\
\midrule
...
%====================================================================
% Tabela 8 — childhoodHobbiesTime
%====================================================================
\begin{table}[H]
\centering
\caption{Distribuição da feature \textit{childhoodHobbiesTime} por cluster}
\label{tab:childhoodHobbiesTime}
\begin{tabular}{lcccc}
\toprule
\textbf{Cluster} & \textbf{0.00} & \textbf{0.33} & \textbf{0.66} & \textbf{1.00} \\
\midrule
...
\begin{tabular}{lcc}
\toprule
\textbf{Cluster} & \textbf{Masculino (0)} & \textbf{Feminino (1)} \\
\midrule
1 & 1 & 22 \\
2 & 27 & 38 \\
\bottomrule
...
\begin{figure}[h!]
    \centering
    \caption{Legenda}
    \includegraphics[width=0.9\textwidth]{geral_stem/aranha_geral_gender_stem.png}
    \label{fig:shap_masculino_future}
\end{figure}
