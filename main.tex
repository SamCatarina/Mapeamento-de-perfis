...
\begin{center}
\begin{longtable}{|p{0.6cm}|p{6.0cm}|p{2.1cm}|p{3.0cm}|F{2.0cm}|}
\caption{Fichamento das questões – Informações Pessoais} \\
\hline
\textbf{\#} & \textbf{Questionamento} & \textbf{Tipo} & \textbf{Opções} & \textbf{Feature} \\
\hline
\endfirsthead
...
1 & Qual gênero você se identifica? 
  & Seleção única 
  & Masculino; Feminino; Outro 
  & gender \\
\hline

2 & Você atualmente está em uma universidade... 
  & Seleção única 
  & Pública; Privada 
  & universityType \\
\hline

3 & Qual a área geral do seu curso?
  & Seleção única
  & Exatas; Humanas; Biológicas; Sociais; Outro
  & courseAreaGeneral \\
\hline
...
5 & Maior escolaridade dos seus pais
  & Seleção única
  & Fund. incompleto; Fund. completo; Médio; Superior; Pós; Outro; Não sei
  & parentsEducation \\
\hline

6 & Tipo de escola cursada no fundamental e médio
  & Seleção única
  & Pública/Pública; Privada/Privada; Pública/Privada; Privada/Pública
  & basicSchoolType \\
\hline

7 & Na infância e adolescência, você tinha tempo e condições para praticar hobbies?
  & Seleção única
  & Sempre; Às vezes; Raramente; Nunca
  & childhoodHobbiesTime \\
\hline

8 & Principais atividades de interesse na infância e adolescência
  & Múltipla escolha
  & Lista selecionada
...
\end{table}
As principais observações a partir do gráfico de SHAP geral com ambos os gêneros são:
- genero, que representa a variável alvo do modelo, codificada como 0 para feminino e 1 para masculino, é mostrando, dentro do shap, como a coluna de feature value, indicvado eplas cores vermelha (valores altos, no caso masculino) e azul (valores baixos, no caso feminino). Ou seja, no gráfico, os pontos azuis (valores baixos) estão associados ao gênero feminino, enquanto os pontos vermelhos (valores altos) estão associados ao gênero masculino.
- percepção que o shap values mostra que o sentimento de exclusão em tech é  identificado como o mais importante para predizer o gênero, onde os valores mais altos dessa variável (indicando maior sentimento de exclusão) estão associados ao gênero feminino, enquanto os valores mais baixos (indicando menor sentimento de exclusão) estão associados ao gênero masculino. (esta de acrodo com os estudos achados)
- a segunda variavel identificada foi a de restrição de atividades por questão de genero, que foi visto que a maioria dos respondentes para valores mais altos indicando que ja deixaram de realizar alguma atividade(valores baixos dessa variavel) pro questão de genero é predominantemente masculina, (--explica pelo teor da questão, ja que mostra diferente dos trabalhos, pode ter sido interpretado de forma diferente)
- A terceira variável mais importante é a opinião sobre profissões por gênero, onde valores mais altos (indicando maior concordância com estereótipos de gênero em profissões) estão associados ao gênero masculino, enquanto valores mais baixos (indicando menor concordância com esses estereótipos) estão associados ao gênero feminino.
\subsection{analise com genero feminino}
...
\subsection{analise com genero masculino}
...
\begin{figure}[h!]
    ...
    \label{fig:dendrograma_geral_gender}
\end{figure}
...
\begin{figure}[h!]
    ...
    \label{fig:cotovelo_geral_gender}
\end{figure}
